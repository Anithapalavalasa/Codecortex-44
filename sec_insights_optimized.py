...
def get_sample_charts_data():
    ...
    return {
        "filings_per_year": pd.DataFrame({'Year': years, 'Filings': filings}),
        "questions_per_company": pd.DataFrame({'Company': companies, 'Questions': questions}),
        "section_coverage": pd.DataFrame({'Section': sections, 'Coverage': coverage})
    }

@st.cache_resource
def initialize_rag():
    """Initialize the RAG components with updated chain - optimized version"""
    ...
    try:
        ...
        # Load embeddings with optimized settings
        embeddings = HuggingFaceEmbeddings(
            model_name="sentence-transformers/all-MiniLM-L6-v2",
            encode_kwargs={'normalize_embeddings': True}
        )
        
        # Load vector store with optimized settings
        vectorstore = Chroma(
            persist_directory=DB_PATH,
            embedding_function=embeddings
        )
        
        # Optimized retriever with reduced search results
        retriever = vectorstore.as_retriever(search_kwargs={"k": 2})  # Reduced from 3 to 2
        
        # Local LLM with timeout settings
        llm = None
        try:
            # Try mistral first, then fall back to llama3
            try:
                llm = Ollama(model="mistral", temperature=0.7, timeout=30)
                test_response = llm.invoke("Hello")
            except:
                llm = Ollama(model="llama3", temperature=0.7, timeout=30)
                test_response = llm.invoke("Hello")
        except Exception as e:
            # Create a simple mock LLM for demonstration
            mock_responses = [
                "Based on the SEC filing data, the company reported strong financial performance. Key highlights include increased market share and successful expansion. For specific details, please ensure Ollama is running with the mistral or llama3 model.",
                "The company's annual report indicates significant investments in research and development. Their strategic initiatives focus on digital transformation. To get precise figures, please start Ollama with either the mistral or llama3 model.",
                "According to the filing documents, the company maintains a strong balance sheet. Their debt-to-equity ratio remains within industry benchmarks. For comprehensive analysis, please enable the local LLM by running Ollama."
            ]
        ...
        # Create the RAG chain (optimized approach)
        qa_chain = (
...
with tab1:
    ...
    with col1:
        ...
        st.markdown(f'<div class="metric-value">{metrics["total_filings"]}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col2:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.markdown('<div class="metric-label">Companies</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-value">{metrics["companies"]}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col3:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.markdown('<div class="metric-label">Questions Asked</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-value">{metrics["questions_asked"]}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col4:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.markdown('<div class="metric-label">Model Used</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-value">{metrics["model_used"]}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col5:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.markdown('<div class="metric-label">Avg. Response</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-value">{metrics["avg_response"]}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    
    st.markdown('</div>', unsafe_allow_html=True)
    
    # Charts section using cached data
    chart_data = get_sample_charts_data()
    ...
    with col1:
        ...
        st.bar_chart(chart_data["filings_per_year"].set_index('Year'))
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col2:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown("### 📊 Question Count per Company")
        st.bar_chart(chart_data["questions_per_company"].set_index('Company'))
        st.markdown('</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with st.form(key="chat_form", clear_on_submit=True):
        ...
        if submit_button and user_input:
            ...
            # Generate response with progress indication
            if not PACKAGES_AVAILABLE:
                ...
                try:
                    ...
                    with st.spinner("Analyzing SEC filings..."):
                        ...
                        confidence = min(95, max(70, 95 - processing_time))  # Adjust confidence based on speed
                        sources = [
...
chart_data = get_sample_charts_data()
st.bar_chart(chart_data["section_coverage"].set_index('Section'))
