...
def initialize_rag():
    ...
    try:
        ...
        try:
            ...
            llm = FakeListLLM(responses=["This is a mock response. Install Ollama and required packages for real answers."])
        
        # Create prompt template
        template = """
        You are a financial analyst specializing in SEC filings. Use the following context to answer the question.
...
with tab3:
    ...
    with col1:
        company = st.selectbox("Select Company (Clean)", ["Apple Inc.", "Microsoft Corp.", "Amazon.com Inc.", "Google LLC", "Tesla Inc.", "Meta Platforms Inc.", "NVIDIA Corp.", "Netflix Inc."], key="company_clean")
    with col2:
        doc_type = st.selectbox("Document Type (Clean)", ["10-K", "10-Q", "8-K", "S-1"], key="doc_type_clean")
    with col3:
        year = st.selectbox("Filing Year (Clean)", ["2023", "2022", "2021", "2020", "2019", "2018"], key="year_clean")
    ...
    if st.session_state.selected_documents:
        ...
        for i, doc in enumerate(st.session_state.selected_documents):
            ...
            with col2:
                if st.button("❌", key=f"remove_clean_{i}"):
            ...
            if not st.session_state.selected_documents:
                ...
                try:
                    with st.spinner("Analyzing SEC filings..."):
                        # Simulate response generation for demo
...
# Footer
st.markdown("<div class='footer'>SEC Filing Summarizer & Q&A (RAG) | Hackathon Project</div>", unsafe_allow_html=True)